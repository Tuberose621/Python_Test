...
newTest = Blueprint('newTest', __name__, template_folder='templates')


@newTest.route('/newTest/login', methods=['GET', 'POST'])
def login():
    form = LoginForm()
...
@newTest.route('/newTest/logout', methods=['GET', 'POST'])
def logout():
    logout_user()
    if 'admin'in session:
        session.pop('admin')
...
@newTest.route('/newTest/verify-code.jpg')
def verify_code():
...
# i am low bee
@newTest.route('/newTest/reset', methods=['GET', 'POST'])
def reset():
    if request.method == 'GET':
...
@newTest.route('/newTest/reset-success', methods=['GET'])
def reset_success():
...
@newTest.route('/newTest/change-password', methods=['POST'])
def change_password():
    params = request.form or request.get_json()
    try:
        username = params['username']
...
@newTest.route('/newTest/check-verification', methods=['POST'])
